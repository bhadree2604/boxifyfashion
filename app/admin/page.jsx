...
export default function AdminPage() {
  ...
  const handleEmailLogin = async (e) => {
    ...
    setEmailLoginError('');
    if (!email || !password) {
      setEmailLoginError('Please enter both email and password.');
      return;
    }
    if (!isFirebaseConfigured || !auth) {
      setEmailLoginError('Firebase credentials are not set in .env.local!');
      return;
    }
    ...
    try {
      ...
      const allowedEmailsStr = process.env.NEXT_PUBLIC_ADMIN_EMAILS;
      if (!allowedEmailsStr) {
        setEmailLoginError('Admin emails not configured. Please set NEXT_PUBLIC_ADMIN_EMAILS in .env.local');
        await signOut(auth);
      } else {
        const allowedEmails = allowedEmailsStr
          .split(',')
        ...
        if (!allowedEmails.includes(userEmail)) {
          await signOut(auth);
          setEmailLoginError('Unauthorized account. Access denied.');
        }
      }
    } catch (err) {
      ...
      if (err.code === 'auth/invalid-credential' || err.code === 'auth/wrong-password' || err.code === 'auth/user-not-found') {
        ...
        setEmailLoginError(`Login failed: ${err.message}`);
      }
    } finally {
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <form onSubmit={handleEmailLogin} style={{ display: 'grid', gap: '1rem', marginTop: '1.5rem' }}>
            {emailLoginError && <div className="admin-error-banner">{emailLoginError}</div>}
            <label className="admin-form-group">
              ...
              <input
                ...
                placeholder="admin@boxifyfashion.com"
                value={email}
            ...
            <button className="btn solid" type="submit" disabled={emailLoggingIn} style={{ width: '100%' }}>
              {emailLoggingIn ? 'Signing in...' : 'Sign In'}
            </button>
          </form>
    ...
    <div className="page-main">
      <section className="grid">
        <div className="admin-header-bar">
          ...
          <div className="cta-row">
            <button className="btn solid" type="button" onClick={openAddModal}>
              + Add New Product
            </button>
      ...
      <section className="grid">
        <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2>Live Products ({productsList.length})</h2>
